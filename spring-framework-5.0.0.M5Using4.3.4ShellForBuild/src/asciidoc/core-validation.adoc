...
=== 配置ConversionService(Configuring a ConversionService)
...
ConversionService是一种无状态对象，旨在在应用程序启动时实例化，然后在多个线程之间共享。在Spring应用程序中，您通常为每个Spring容器（或ApplicationContext）配置ConversionService实例。该转换服务将被Spring接收，然后在需要框架执行类型转换时使用。您也可以将此ConversionService注入任何一个bean并直接调用。
...
要使用Spring注册一个默认的ConversionService，请添加以下代码为 `convertService` 的bean定义：
...
默认的ConversionService可以在字符串，数字，枚举，集合，map和其他常见类型之间进行转换。要使用自己的自定义转换器补充或覆盖默认转换器，请设置 `转换器` 属性。属性值可以实现Converter，ConverterFactory或GenericConverter接口。
...
在Spring MVC应用程序中使用ConversionService也是常见的。请参见Spring MVC章节中的<<mvc-config-conversion>>。
...
=== 以编程方式使用ConversionService(Using a ConversionService programmatically)
...
对于大多数用例，可以使用指定 _targetType_ 的 `convert` 方法，但它不适用于更复杂的类型，例如参数化元素的集合。例如，如果要以编程方式将 `整数` 的 `List` 转换为 `字符串` 的 `List`，则需要提供源和目标类型的正式定义。

Fortunately, `TypeDescriptor` provides various options to make that straightforward:

幸运的是，`TypeDescriptor` 提供了多种选择，使之简单化：
...
----
	...
	List<Integer> input = ....
	cs.convert(input,
		TypeDescriptor.forObject(input), // List<Integer>类型描述符(List<Integer> type descriptor)
...
请注意，`DefaultConversionService` 会自动注册转换器，适用于大多数环境。这包括集合转换器，标量转换器，以及基本的 `Object` 到 `String` 转换器。在 `DefaultConversionService` 类中，可以使用 _static_ `addDefaultConverters` 方法向任何 `ConverterRegistry` 注册相同的转换器。
...
值类型的转换器将被重用于数组和集合，因此，假设标准集合处理是合适的，则不需要创建一个特定的转换器来将其从 `S` 的 `集合` 转换为 `T` 的 `集合`。




[[format]]
== Spring字段格式化(Spring Field Formatting)
...
如上一节所讨论的，<<core-convert, `core.convert`>>是一个通用类型转换系统。它提供了一个统一的ConversionService API以及强类型转换器SPI，用于实现从一种类型到另一种类型的转换逻辑。Spring容器使用此系统来绑定bean属性值。此外，Spring表达式语言（Spel）和DataBinder都使用此系统来绑定字段值。例如，当SpEL需要强制转换 `Short` 到 `Long` 以完成 `expression.setValue(Object bean，Object value)` 尝试时，core.convert系统执行强制转换。
...
现在考虑典型客户端环境（如Web或桌面应用程序）的类型转换要求。在这种环境中，您通常会__转换String__以支持客户端回发过程，以及__转换回String__以支持视图呈现过程。另外，您经常需要本地化String值。更一般的__core.convert__转换器SPI不直接解决这种__格式化__要求。为了直接解决它们，Spring 3引入了一个方便的格式化程序SPI，为客户端环境提供了一个简单而强大的PropertyEditor替代方案。
...
一般来说，当需要实现通用类型转换逻辑时，请使用转换器SPI；例如，用于在java.util.Date和java.lang.Long之间进行转换。当您在客户端环境（如Web应用程序）中工作，并需要解析和打印本地化的字段值时，请使用格式化程序SPI。ConversionService为两个SPI提供统一的类型转换API。



[[format-Formatter-SPI]]
=== 格式化SPI(Formatter SPI)
The Formatter SPI to implement field formatting logic is simple and strongly typed:

格式化SPI实现字段格式化逻辑是简单和强类型的：
...
Formatter从Printer和Parser构建块接口中继承的位置：
...
要创建您自己的Formatter，只需实现上面的Formatter接口。参数化T为要格式化的对象的类型，例如 `java.util.Date`。实现 `print()` 操作来打印一个T的实例来以客户端语言环境显示。实现 `parse()` 操作，从客户端语言环境返回的格式化表达来解析T的一个实例。如果解析尝试失败，则您的Formatter应抛出ParseException或IllegalArgumentException异常。注意确保您的Formatter实现是线程安全的。
...
为了方便起见，`format` 子包中提供了几个Formatter实现。`number` 包提供了一个 `NumberFormatter`，`CurrencyFormatter` 和 `PercentFormatter` 来使用 `java.text.NumberFormat` 来格式化 `java.lang.Number` 对象。`datetime` 包提供了一个 `DateFormatter`，用 `java.text.DateFormat` 格式化 `java.util.Date` 对象。`datetime.joda` 包提供了基于 http://joda-time.sourceforge.net[Joda Time库]的全面的datetime格式支持。

Consider `DateFormatter` as an example `Formatter` implementation:

以 `DateFormatter` 为 `Formatter` 实现的例子：
...
Spring团队欢迎社区驱动的 `Formatter` 贡献；参见 https://jira.spring.io/browse/SPR[jira.spring.io]来贡献。
...
如您所见，字段格式化可以通过字段类型或注解进行配置。要将注解绑定到格式化程序，请实现AnnotationFormatterFactory：
...
参数化A为要将格式化逻辑与之相关联的字段注解类型，例如 `org.springframework.format.annotation.DateTimeFormat`。使 `getFieldTypes()` 返回可以使用注解的字段类型。使 `getPrinter()` 返回一个Printer打印被注解的字段的值。使 `getParser()` 返回一个Parser来解析一个被注解的字段的客户端值。
...
下面的示例AnnotationFormatterFactory实现将@NumberFormat注解绑定到格式化程序。此注解允许指定数字样式或模式：
...
`org.springframework.format.annotation` 包中存在轻便的格式注解API。使用 @NumberFormat 格式化java.lang.Number字段。使用 @DateTimeFormat 格式化java.util.Date，java.util.Calendar，java.util.Long或Joda时间字段。
...
FormatterRegistry是用于注册格式化程序和转换器的SPI。`FormattingConversionService` 是适用于大多数环境的FormatterRegistry的实现。该实现可以使用 `FormattingConversionServiceFactoryBean` 以编程方式或声明式配置为Spring bean。由于此实现还实现了 `ConversionService`，因此可以直接配置为与Spring的DataBinder和Spring表达式语言（Spel）配合使用。
...
如上所示，可以通过fieldType或注解来注册Formatter。
...
FormatterRegistry SPI允许您集中配置格式化规则，而不是在控制器上复制此类配置。例如，您可能要强制以某种方式将所有日期字段格式化，或将具有特定注解的字段以某种方式进行格式化。使用共享的FormatterRegistry，您可以仅定义这些规则一次，并且在需要格式化时应用它们。
...
FormatterRegistrar是通过FormatterRegistry注册格式化程序和转换器的SPI：
...
在为特定格式化类别注册多个相关转换器和格式化程序（如日期格式化）时，FormatterRegistrar很有用。在声明性注册不足的情况下也是有用的。例如，当格式化程序需要在与其自己的<T>不同的特定字段类型下被索引时，或者在注册Printer/Parser对时。下一节提供有关转换器和格式化程序注册的更多信息。



[[format-configuring-formatting-mvc]]
=== 在Spring MVC中配置格式化(Configuring Formatting in Spring MVC)

See <<mvc-config-conversion>> in the Spring MVC chapter.

请参见Spring MVC章节中的<<mvc-config-conversion>>。




[[format-configuring-formatting-globaldatetimeformat]]
== 配置全局日期和时间格式(Configuring a global date & time format)
...
默认情况下，未使用 `@DateTimeFormat` 注解的日期和时间字段将使用 `DateFormat.SHORT` 样式从字符串转换。如果你愿意，你可以通过定义你自己的全局格式来改变它。
...
您将需要确保Spring不注册默认格式化程序，而应手动注册所有格式化程序。使用
 `org.springframework.format.datetime.joda.JodaTimeFormatterRegistrar` 或 `org.springframework.format.datetime.DateFormatterRegistrar` 类，取决于你是否使用Joda Time库。
